...
void PresentationFeedbacks::presented(KWin::RenderLoop *loop, std::chrono::nanoseconds current)
{
    if (!this->isactive()) {
        return;
    }
    ...
    KWin::RenderLoop::VrrPolicy vrr = loop->vrrPolicy();
    std::chrono::nanoseconds next = loop->nextPresentationTimestamp();

    uint32_t nSecToNextRefresh = 0; // On VRR, nextrefresh need to be set to 0
    if (vrr == KWin::RenderLoop::VrrPolicy::Never) {
        nSecToNextRefresh = uint32_t(next.count() - current.count());
    }

    PresentationFeedbackInterface::Kinds ret;
    if (m_surface->presentationHint() == PresentationHint::VSync) {
