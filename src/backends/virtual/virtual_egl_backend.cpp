    ...
    , m_size(size)
{
    ...
    for (int i = 0; i < 2; ++i) {
        GbmGraphicsBuffer *buffer = allocator.allocate(size, format);
        if (!buffer) {
            qCWarning(KWIN_VIRTUAL) << "Failed to allocate layer swapchain buffer";
            continue;
        }
        m_buffers.append(std::make_shared<VirtualEglLayerBuffer>(buffer, backend));
    }
}

VirtualEglSwapchain::~VirtualEglSwapchain()
{
}
